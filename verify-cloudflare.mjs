...
for (const path of ['/styles.css', '/dashboard.css', '/api.js', '/scripts.js', '/dashboard-v2.js', '/dashboard-loop.js', ...moduleAssets, '/vendor/echarts/echarts.min.js', '/assets/maps/northeast-china-admin1.geojson', '/assets/maps/changchun-service-area.geojson', '/assets/maps/changchun-road-basemap.v1.geojson', '/assets/backgrounds/northeast-winter-corn-v1.webp']) {
...
const roadBasemap = await (await requireOk('/assets/maps/changchun-road-basemap.v1.geojson')).json();
const roadLayers = new Set((roadBasemap.features || []).map((feature) => feature.properties?.layer));
if (roadBasemap?.metadata?.crs !== 'EPSG:4326' || roadBasemap?.metadata?.license !== 'ODbL 1.0'
  || !['motorway', 'primary', 'secondary', 'railway', 'waterway', 'place_label'].every((layer) => roadLayers.has(layer))) {
  throw new Error('The local Changchun road basemap failed its layer or license contract.');
}
