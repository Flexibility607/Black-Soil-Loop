...
def test_dashboard_snapshot_splits_units_channels_and_coordinates(client: TestClient, db_session) -> None:
    ...
    assert snapshot["headline"]["preorder_count"] == 3
    ...
    assert snapshot["data_quality"]["excluded_preorder_count"] == 1
    assert snapshot["data_quality"]["excluded_demand_totals"] == [{"unit": "公斤", "quantity": 999.0}]
    assert snapshot["data_quality"]["report_coverage"] == 33.3
