import { encodePcm16Wav } from './dashboard-wav-worker.js?v=20260811-anonymous-voice-1';
  ...
  if (context.audioWorklet?.addModule && AudioWorkletNodeClass) {
    try {
      const moduleUrl = options.workletModuleUrl
        || new URL('./dashboard-audio-worklet.js?v=20260811-anonymous-voice-1', import.meta.url).href;
...
export function encodeWavInWorker(chunks, sampleRate, { requestId, signal, WorkerClass = globalThis.Worker } = {}) {
  ...
  try {
    worker = new WorkerClass(
      new URL('./dashboard-wav-worker.js?v=20260811-anonymous-voice-1', import.meta.url),
