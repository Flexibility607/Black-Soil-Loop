...
async def receive_b02_event(
    ...
    if content_length and content_length.isdigit() and int(content_length) > settings.b02_max_body_bytes:
        raise HTTPException(status_code=413, detail={"code": "BODY_TOO_LARGE", "message": "B02 事件正文超过限制"})
    body = await request.body()
    if len(body) > settings.b02_max_body_bytes:
        raise HTTPException(status_code=413, detail={"code": "BODY_TOO_LARGE", "message": "B02 事件正文超过限制"})
    authenticate_b02_request(
        db,
        settings,
